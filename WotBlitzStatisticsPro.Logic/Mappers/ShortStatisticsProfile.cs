        ...
        public ShortStatisticsProfile()
        {
            CreateMap<IStatistics, ShortStatistics>()
                ...
                .ForMember(d => d.AvgDamage,
                    o => o.MapFrom(s => s.Battles.HasValue && s.Battles > 0 ? ((decimal)s.DamageDealt / s.Battles) : 0m))
                .ForMember(d => d.AvgXp,
                    o => o.MapFrom(s => s.Battles.HasValue && s.Battles > 0 ? ((decimal)s.Xp / s.Battles) : 0m))
                .ForMember(d => d.WinRate,
                    o => o.MapFrom(s => s.Battles.HasValue && s.Battles > 0 ? (100 * (decimal)s.Wins / s.Battles) : 0m))
                ;
        }
    }
}